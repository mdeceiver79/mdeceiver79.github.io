...
const ruleOverheat = Registry.addRule("Overheat", "Death if 3 or more 1's are rolled at once.", "If 3 or more 1's are rolled at once the beater of this weapon dies. ");
...
const npcPlagueZombie = Registry.addNpc("Plague Zombie", "A shambling victim of Nurgle's gifts. Slow but hard to kill, a threat in large numbers.", "Grunt", factionNurgle, 3, "1D6-2", [], [ruleWeaknessFire, ruleFeelNoPain3]);
...
const npcPlagueMarine = Registry.addNpc("Plague Marine", "A bloated Chaos Space Marine blessed by Nurgle. Hard to kill, considered a threat.", "Heavy", factionNurgle, 4, "1D6", [weaponBolter, weaponPlagueBomb], [ruleBulky, rulePlagueWeapons, ruleFeelNoPain3]);
...
const npcChaosSpawn = Registry.addNpc("Chaos Spawn", "", "Heavy", factionChaos, 5, "3D6", [], [ruleBulky, ruleFeelNoPain3]);


const npcUrGhul = Registry.addNpc("Ur Ghul", "", "Grunt", factionBeasts, 6, "1D6", [], []);
const npcAmbull = Registry.addNpc("Ambull", "", "Boss", factionBeasts, 4, "2D6", [], [ruleBulky, ruleHardToKill]);
const npcDog = Registry.addNpc("Guard Dog", "Dogs have long accompanied humans for hunting, protection and companionship.", "Grunt", factionBeasts, 5, "1D6-2", [], [ruleUnarmored]);
const npcHuntingBeast = Registry.addNpc("Hunting Beast", "Large beasts with thick hides", "Grunt", factionBeasts, 5, "1D6", [], [ruleFlakArmour]);
const npcEnslaver = Registry.addNpc("Enslaver", "Telepathic slavers", "Heavy", factionBeasts, 4, "1D6", [], [ruleFlakArmour]);
const npcBorewyrm  = Registry.addNpc("Borewyrm", "Lil worms", "Grunt", factionBeasts, 4, "1D6-1", [], []);
const npcEyestingerSwarm = Registry.addNpc("Eyestinger Swarm", "Giant mutated parasitic insects, they will swarm a target to lay eggs. If successful in this endevour the target's flesh will rupture with fash growing maggots which will then emerge as more flies.", "Grunt", factionBeasts, 4, "1D6-2", [], [ruleWeaknessFire]);
const npcFlea = Registry.addNpc("Flea", "Giant mutated fleas, drawn to the prospect of blood. Between their speed, their thick carapace and their general hardiness they can present a challenge. Capable of draining a human sized target of blood in several minutes they can swell up to 3 times their size, they will drain a target of blood before moving on.", "Grunt", factionBeasts, 4, "1D6-1", [], [ruleWeaknessFire, ruleLeap]);
const npcGrub = Registry.addNpc("Grub", "Carnivorous giant grubs, they latch onto their prey poisoning them for later consumption. They can also spit short distances.", "Grunt", factionBeasts, 4, "1D6-2", [weaponPoisonSpit], [ruleWeaknessFire, rulePoison]);


const npcHullghast = Registry.addNpc("Hullghast", "The Hullghast are mutants found aboard voidships. They share an origin with the Ghilliam, as both types are created by irradiated or polluted voidship decks. They have hairless, leathery skin, vicious talon-like claws, oversized mouths filled with layers of huge teeth, pustules of flesh erupting in tusks, and horns. Unlike the Ghilliam they are far more aggressive, and prefer feeding on crewmen that go too deep below decks; when unable to do so, they feed on each other.", "Heavy", factionBeasts, 5, "1D6", [], [ruleBulky, ruleFeelNoPain5]);
const npcGhilliam = Registry.addNpc("Ghilliam", "Mutated Ghoul", "Grunt", factionBeasts, 4, "1D6-1", [], [ruleUnarmored]);

const npcCivilian = Registry.addNpc("Civilian", "", "Grunt", factionCivilians, 4, "1D6-3", [], [ruleUnarmored]);
const npcRabble = Registry.addNpc("Armed Rabble", "When civilians are pushed to defend themselves they group together and find whatever weapons they have available - but with little training they're unlikely to achieve much.", "Grunt", factionCivilians, 4, "1D6-3", [weaponSmallArms], [ruleUnarmored]);

const npcTechThrall = Registry.addNpc("Tech Thrall", "Human puppets controlled by a machine with nothing but base instincts and a withered sentience.", "Grunt", factionDarkMechanicum, 4, "1D6-1", [], [ruleFlakArmour]);
